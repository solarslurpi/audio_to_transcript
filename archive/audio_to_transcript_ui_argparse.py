...
def parse_arguments():
    ...
    parser.add_argument(
        "--youtube_url", type=str, help="YouTube Video or Playlist URL", required=False
    )
    parser.add_argument(
        "--mp3_filename",
        type=str,
        help="MP3 file path for transcription",
        required=False,
    )
    parser.add_argument(
        "--transcript_folder",
        type=str,
        help="Folder path to save transcripts",
        required=True,
    )
...
def main():
    ...
    try:
        transcript_folder = (
            ...
            if args.transcript_folder
            else os.path.join(os.getcwd(), "transcripts")
        )

        if not os.path.exists(transcript_folder):
            print(
                f"Transcript folder '{transcript_folder}' does not exist. Creating it."
            )
    ...
    if args.youtube_url:
        ...
        mp3_filename = audio_transcriber.download_youtube_audio(
            args.youtube_url, transcript_folder
        )
